...
- The Build Upload API intermittently answers 404 `NOT_FOUND` ("There is no resource of type 'apps' with id '…'") for apps that certainly exist, right after app creation and at random; fastlane hit the same flake after moving build-number lookups to `GET /v1/apps/{id}/buildUploads` (https://github.com/fastlane/fastlane/issues/29908). GET/HEAD requests to `/v1/apps/{id}/buildUploads` and `/v1/apps/{id}/relationships/buildUploads` therefore treat any 404 `NOT_FOUND` as transient (the app is the only resource that can be missing there), and GET/HEAD requests to `/v1/buildUploads*` and `/v1/buildUploadFiles*` do so only when Apple's detail names resource type `apps`; all of these reuse the shared retry policy (`ASC_MAX_RETRIES`, `ASC_BASE_DELAY`, `ASC_MAX_DELAY`, `ASC_RETRY_LOG`; 3 retries at roughly 1s/2s/4s by default). A 404 that survives the retry budget is surfaced with Apple's original detail, exit code, and `retry limit exceeded` prefix. A genuinely missing upload or file id (`There is no resource of type 'buildUploads' ...`), writes (`POST`/`PATCH`/`DELETE`) on those paths, 404s with any other error code, and 404s on every other endpoint are still never retried and keep their original error shape. This affects `asc builds upload`, `asc builds next-build-number`, `asc builds uploads *`, and the publish flows that poll build uploads.
